  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "relative z-50 w-full max-w-lg rounded-xl bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto pointer-events-auto",
          className,
        )}
        {...props}
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-4 top-4 rounded-sm opacity-70 hover:opacity-100 transition-opacity focus-visible:outline-2 focus-visible:outline-accent"
          aria-label="Cerrar"
        >
          <X className="h-4 w-4" />
        </button>
...
function DialogClose({ onClose, className }: { onClose: () => void; className?: string }) {
  return (
    <button
      ...
      aria-label="Cerrar"
      className={cn(
        "absolute right-4 top-4 rounded-sm opacity-70 hover:opacity-100 focus-visible:outline-2 focus-visible:outline-accent",
        className,
      )}
    >
      <X className="h-4 w-4" />
